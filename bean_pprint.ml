(* pretty-printer converts from bean source code to well-formed style *)
open Bean_ast
open Format

(* All boxes except typedef parts, are vertical box(@[<v>) @], *)
(* which means every break hint(@, or @;<0 4>) leads to a new line *)

(*@;<0 4> break hint means indent 4 spaces for following lines *)
...
and print_tspec fmt = function
  | Ttype Bool -> fprintf fmt "bool"
  | Ttype Int -> fprintf fmt "int"
  | Tfdef fdefs -> fprintf fmt "{%a}" print_fdefs fdefs
  | Tid ident -> fprintf fmt "%s" ident
...
and print_proc fmt (pheader, pbody) = 
  ...
  print_newline ();
  (* indent 4 spaces for declarations and top-level statements *)
  (* within each procudure *)
  fprintf fmt "@[<v>proc %a@;<0 4>@[<v>%a@]@,end@]@." 
    print_pheader pheader print_pbody pbody
  
and print_pheader fmt (ident, params) = 
  fprintf fmt "%s(%a)" ident print_params params
  
and print_params fmt = function
  | [] -> ()
  | [x] -> print_param fmt x
  | x :: xs -> fprintf fmt "%a, " print_param x; print_params fmt xs
  
and print_param fmt (indicator, typespec, ident) = 
  match indicator with
  | Val -> fprintf fmt "val %a %s" print_tspec typespec ident
  | Ref -> fprintf fmt "ref %a %s" print_tspec typespec ident
